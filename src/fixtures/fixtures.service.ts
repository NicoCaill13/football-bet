...
export class FixturesService {
  ...
  async logReschedule(matchId: number, from: Date, to: Date, note?: string) {
    await this.prisma.fixtureChangeLog.create({
      data: { matchId, type: 'reschedule', from, to, note },
    });
    return { ok: true };
  }
  async logPostpone(matchId: number, from: Date, note?: string) {
    await this.prisma.fixtureChangeLog.create({
      data: { matchId, type: 'postpone', from, note },
    });
    return { ok: true };
  }
  async logCancel(matchId: number, from: Date, note?: string) {
    await this.prisma.fixtureChangeLog.create({
      data: { matchId, type: 'cancel', from, note },
    });
    return { ok: true };
  }
  async logVenueChange(matchId: number, note?: string) {
    await this.prisma.fixtureChangeLog.create({
      data: { matchId, type: 'venue', note },
    });
    return { ok: true };
  }

  async history(matchId: number) {
    return this.prisma.fixtureChangeLog.findMany({
      where: { matchId },
  ...
  createFixture(dto: CreateFixtureDto) {
  ...
  listCompetitionFixtures(competitionId: number) {
    return this.byCompetition(competitionId);
  }
  listSeasonFixtures(seasonId: number) {
    return this.bySeason(seasonId);
  }

  reschedule(id: number, dto: RescheduleDto) {
    return this.logReschedule(
      id,
      new Date(dto.from),
      new Date(dto.to),
      dto.note,
