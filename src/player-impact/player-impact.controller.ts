...
export class PlayerImpactController {
  ...
  @ApiQuery({
    name: 'league',
    example: 'L1',
    description: 'Code interne (L1, PL, SA, LL, BUN, ...)',
  })
  @ApiQuery({
    name: 'season',
    example: 2025,
    type: Number,
    description: 'Année de début de saison (ex: 2025)',
  })
  @ApiOkResponse({
    description: 'Construit/MAJ PlayerImpact pour league+season',
  })
  async buildImpact(
    ...
    if (!Number.isFinite(seasonNum))
      throw new BadRequestException('season must be numeric, e.g. 2025');
    if (!league || typeof league !== 'string')
      throw new BadRequestException('league must be a string code, e.g. L1');
