...
export class PredictionService {
  ...
  async getSummary(matchId: number, oddsMode?: string) {
    ...
    const injDelta = await this.tiltInjuries(homeId, awayId, seasonId, startsAt);
    ...
    const totalDelta = eloDelta + xgDelta + restDelta + injDelta + congDelta + lookDelta;
    ...
    const pick = (C.pickRule === 'prob') ? this.pickByProb(adjusted) : this.pickFromEV(ev); 
    const kelly = this.kellyFraction(adjusted, odds, pick);
    
    const suggested = Math.min(C.stake.cap, Math.max(0, kelly));

    const winnerTeam =
      pick === '1' ? match.homeTeam?.name :
      pick === '2' ? match.awayTeam?.name : 'Draw';

    return {
      match: { id: match.id, home: match.homeTeam?.name, away: match.awayTeam?.name, startsAt },
      usingOdds: { mode: use, ...odds },
      probabilities: { market, adjusted },
      prediction: { winner: pick, winnerTeam, probability: adjusted[pick === '1' ? 'p1' : pick === '2' ? 'p2' : 'pX'] },
      drivers: {
        weights: { ...C.weights },
        elo: { homeAdv: C.elo.homeAdv, delta: eloDelta },
        xg: { span: C.xg.spanKey, delta: xgDelta },
        injuries: { lookbackDays: C.injuries.lookbackDays, span: C.injuries.impactSpanKey, delta: injDelta },
      ...
      stake: { suggestedFraction: suggested, note: suggested === C.stake.cap ? `kelly cap=${C.stake.cap}` : 'kelly' },
    };
  }
  ...
  private async tiltElo(homeId: number, awayId: number) {
    const [eh, ea] = await Promise.all([
      this.prisma.eloRating.findFirst({ where: { teamId: homeId }, orderBy: { ratedAt: 'desc' } }),
      this.prisma.eloRating.findFirst({ where: { teamId: awayId }, orderBy: { ratedAt: 'desc' } }),
    ]);
  ...
  private async tiltXg(homeId: number, awayId: number) {
    ...
    const [xh, xa] = await Promise.all([
      this.prisma.xgTeamRolling.findUnique({ where: { teamId_span: { teamId: homeId, span } } }),
      this.prisma.xgTeamRolling.findUnique({ where: { teamId_span: { teamId: awayId, span } } }),
    ]);
  ...
  private async tiltRest(homeId: number, awayId: number, ref: Date) {
    const last = async (tid: number) =>
      this.prisma.match.findFirst({
        where: { OR: [{ homeTeamId: tid }, { awayTeamId: tid }], startsAt: { lt: ref } },
        orderBy: { startsAt: 'desc' },
        select: { startsAt: true },
      });
    const [mh, ma] = await Promise.all([last(homeId), last(awayId)]);
    const dh = mh?.startsAt ? (ref.getTime() - mh.startsAt.getTime()) / 86400000 : C.rest.capDays;
    const da = ma?.startsAt ? (ref.getTime() - ma.startsAt.getTime()) / 86400000 : C.rest.capDays;
    ...
    private async tiltInjuries(homeId: number, awayId: number, seasonId: number | null, ref: Date) {
      ...
      const [ih, ia] = await Promise.all([teamImpact(homeId), teamImpact(awayId)]);
  ...
  private async tiltCongestion(homeId: number, awayId: number, ref: Date) {
    ...
    const count = (tid: number) =>
      this.prisma.match.count({ where: { OR: [{ homeTeamId: tid }, { awayTeamId: tid }], startsAt: { gte: from, lt: ref } } });
    const [nh, na] = await Promise.all([count(homeId), count(awayId)]);

    const score = (n: number) =>
      Math.max(0, Math.min(1, (n - C.congestion.baseline) / Math.max(1, C.congestion.softMax - C.congestion.baseline)));
    const sh = score(nh);
    const sa = score(na);
  ...
  private async tiltLookaheadEurope(homeId: number, awayId: number, ref: Date) {
    ...
    const [eh, ea] = await Promise.all([hasEuroSoon(homeId), hasEuroSoon(awayId)]);
  ...
  private applyDeltaAndDraw(market: Prob, delta: number, drawBump: number): Prob {
    let { p1, pX, p2 } = market;
  ...
  private oddsToProb(odds: OddsSet): Prob {
    const q1 = 1 / odds.o1, qX = 1 / odds.oX, q2 = 1 / odds.o2;
    const s = q1 + qX + q2;
  ...
  private pickFromEV(ev: Record<'1'|'X'|'2', number>): '1'|'X'|'2' {
    let best: '1'|'X'|'2' = '1';
    let v = ev['1'];
    if (ev['X'] > v) { best = 'X'; v = ev['X']; }
    if (ev['2'] > v) { best = '2'; v = ev['2']; }
    return best;
  }
  ...
  private async getOddsForMatch(matchId: number, mode: string): Promise<OddsSet> {
