...
export class ListMatchesQueryDto {
  @ApiPropertyOptional({ description: 'Competition code (L1, PL, SA, ...)', example: 'L1' })
  @IsOptional() @IsString()
  league?: string;

  @ApiPropertyOptional({ description: 'Season start year (YYYY)', example: '2025' })
  @IsOptional() @Type(() => Number) @IsInt()
  season?: number;

  @ApiPropertyOptional({ enum: ['all', 'upcoming', 'past'], default: 'all' })
  @IsOptional() @IsIn(['all', 'upcoming', 'past'])
  scope?: 'all' | 'upcoming' | 'past';

  @ApiPropertyOptional({ description: 'From date (ISO)', example: '2025-08-01' })
