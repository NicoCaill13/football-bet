...
export class TeamsController {
  ...
  @Get(":id/fixtures")
  @ApiOperation({ summary: "Calendrier d'une équipe (filtrable date, statut, type de compétition)" })
  @ApiParam({ name: "id", type: Number })
  @ApiQuery({ name: "from", required: false, example: "2025-08-01T00:00:00.000Z" })
  @ApiQuery({ name: "to", required: false, example: "2026-06-30T00:00:00.000Z" })
  @ApiQuery({ name: "status", required: false, example: "scheduled" })
  @ApiQuery({ name: "type", required: false, enum: ["league", "cup", "europe"] })
  @ApiOkResponse({ type: Object, isArray: true })
  async fixtures(
    ...
    const matches = await this.service["prisma"].match.findMany({
      ...
      include: { competition: true, season: true, round: true, homeTeam: true, awayTeam: true },
    });
    return matches;
  }
}
