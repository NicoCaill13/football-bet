...
export class OddsController {
  ...
  @Get('best')
  @ApiOperation({
    summary: 'Meilleures cotes par issue (o1/oX/o2 max) + probas',
  })
  @ApiNotFoundResponse({ description: 'No odds snapshot for this match' })
  @ApiParam({ name: 'matchId', type: Number })
  ...
  @Get('fair')
  @ApiOperation({
    summary: 'Vue fair odds + EV + double chance',
    description: "Source = latest ou best via query 'use'",
  })
  @ApiNotFoundResponse({ description: 'No odds snapshot for this match' })
  @ApiParam({ name: 'matchId', type: Number })
  @ApiQuery({
  ...
  async fair(
    ...
    const base =
      use === 'best'
        ? await this.service.best(matchId)
        : await this.service.latest(matchId);
    if (!base) return null;
    ...
    return {
      ...
      ev: {
        '1': +(p.p1 * base.o1 - 1).toFixed(4),
        X: +(p.pX * base.oX - 1).toFixed(4),
        '2': +(p.p2 * base.o2 - 1).toFixed(4),
      },
      doubleChance: (() => {
        const arr = [
          { k: '1', v: p.p1 },
        ...
        return {
          legs: arr[0].k + arr[1].k,
          probability: prob,
          fairOdds: +(1 / prob).toFixed(3),
        };
      })(),
    };
  }
